...
def google_search(query):
    print("Google search is being used")
...
def bing_search(query):
    print("Bing search is being used")

    payload = {
        'zone' : BRIGHTDATA_SERP_ZONE,
        'url' : f"https://.com/bing.search?q={requests.utils.quote(query)}&brd_json=1",
        'format' : "raw",
        'country' : "US"
    }
...
def reddit_search(query):
    print("Reddit search is being used")
...
@tool(description="Use chatgpt to get answer to a question.")
def gpt_prompt(query):
    print("Chatgpt is being used")
    ...
    status = requests.get(f"https://api.brightdata.com/datasets/v3/progress/{snapshot_id}", headers=HEADERS).json()['status']

    while status != 'ready':
        time.sleep(5)
    
    data = requests.post(f"https://api.brightdata.com/datasets/v3/snapshot/{snapshot_id}&format=json", headers=HEADERS).json()[0]

    return data["answer_text_markdown"]


@tool(description="Use perplexity to do reasearch on the question.")
def perplexity_prompt(query):
    print("Perplexity is being used")
...
agent = create_react_agent(
    model = llm,
    tools = [google_search, bing_search, reddit_search, x_search, gpt_prompt, perplexity_prompt],
    debug=False,
